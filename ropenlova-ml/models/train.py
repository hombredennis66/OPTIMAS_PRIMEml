...
Usage:
    ...
    python models/train.py --alpha 0.715 --data data/survey_responses.csv
"""
import argparse
import pickle
...
def train(data_path: str, alpha: float, output_path: str):
    ...
    # Cross-validated score for a more honest read on small datasets
    kf = KFold(n_splits=5, shuffle=True, random_state=42)
    cv_scores = cross_val_score(
        Ridge(alpha=alpha), scaler.fit_transform(X), y, cv=kf, scoring="r2"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", type=float, default=0.715)
