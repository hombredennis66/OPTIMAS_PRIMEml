  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(
          "id, user_id, created_at, monthly_allowance, accommodation_type, transport_type, meal_habits, year_of_study",
        )
        .order("created_at", { ascending: false })
        .limit(200);
      if (error) throw error;
              ...
              (p: {
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell className="font-medium">
                    ₹{Math.round(p.predicted_spending).toLocaleString("en-IN")}
                  </TableCell>
                  <TableCell className="text-xs text-muted-foreground">
...
function useDeleteRow(
  table: "survey_responses" | "predictions",
  key: unknown[],
