...
export const Route = createFileRoute("/_authenticated/survey")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Answer a quick lifestyle survey to get a predicted monthly spending amount.",
      },
    ],
  }),
...
const LABELS: Record<string, string> = {
  ...
  mess: "Mess / canteen",
  home_meals: "Home-cooked",
  cafe: "Cafes / eating out",
  mixed: "Mixed",
};
...
function SurveyPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        setModels(rows);
        if (rows.length && selectedModels.size === 0) {
          const firstId = rows[0]?.id;
          if (firstId) setSelectedModels(new Set([firstId]));
        }
      });
  ...
  const onSubmit = async (values: SurveyFormValues) => {
    ...
    if (sErr || !survey) {
      setSubmitting(false);
      toast.error(sErr?.message ?? "Failed to save survey");
      return;
    }
    ...
    toast.success(
      chosen.length > 1 ? `${chosen.length} predictions generated!` : "Prediction generated!",
    );
    ...
    <div className="mx-auto max-w-3xl px-4 py-10">
      <div className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">
          All values are for a typical month. Takes ~1 minute.
        </p>
      </div>
      ...
      <Card className="p-6 md:p-8">
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-5 md:grid-cols-2">
          <NumberField
            ...
            error={err.monthly_allowance?.message}
            {...reg("monthly_allowance")}
          />
          <NumberField
            label="Distance from campus (km)"
            error={err.distance_from_campus?.message}
            step="0.1"
            {...reg("distance_from_campus")}
          />
          ...
          <SelectField
            ...
            onChange={(v) =>
              form.setValue("accommodation_type", v as SurveyFormValues["accommodation_type"])
            }
            options={accommodationTypes.map((v) => ({ value: v, label: LABELS[v] ?? v }))}
          />
          <SelectField
            label="Transport type"
            value={form.watch("transport_type")}
            onChange={(v) =>
              form.setValue("transport_type", v as SurveyFormValues["transport_type"])
            }
            options={transportTypes.map((v) => ({ value: v, label: LABELS[v] ?? v }))}
          />
          <SelectField
            label="Meal habits"
          ...
          <NumberField
            label="Outings per month"
            error={err.outings_per_month?.message}
            {...reg("outings_per_month")}
          />
          <NumberField
            label="Gaming hours per week"
            error={err.gaming_hours?.message}
            step="0.5"
            {...reg("gaming_hours")}
          />
          <NumberField
            label="Club / society events"
            error={err.club_events?.message}
            {...reg("club_events")}
          />
          <NumberField
            label="Mobile data (GB / month)"
            error={err.mobile_data_usage?.message}
            step="0.5"
            {...reg("mobile_data_usage")}
          />
          <NumberField
            label="Year of study"
            error={err.year_of_study?.message}
            {...reg("year_of_study")}
          />
          <NumberField
            label="Printing pages / week"
            error={err.printing_frequency?.message}
            {...reg("printing_frequency")}
          />

          <div className="md:col-span-2 pt-2">
            <Label className="text-sm">Run prediction with model(s)</Label>
            <p className="text-xs text-muted-foreground mb-2">
              Pick one or more to compare side by side on your dashboard.
            </p>
